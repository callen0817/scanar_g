    ...
    def handle_image(self, msg):
        ...
        if len(self.prev_kpts_3d) >= 4:
            ...
            if success:
                ...
                # Convert camera rotation in world to quaternion
                q_vis = self.rot_to_quat(R_vis.T)
                
                # EKF measurement update for position and orientation
                K_gain = 0.3
                self.state_pos = (1.0 - K_gain) * self.state_pos + K_gain * p_vis
                self.state_q = self.quat_lerp(self.state_q, q_vis, K_gain)
